...
def get_dataset_splits(dataset,binary,pca_dims,data_amount,public_data_amount,do_attack,attack_test_samples,attack_train_samples):
    if dataset.startswith("cifar"):
        num_channels = 3
        image_size = 32
        train_images, train_labels, test_images, test_labels = cifar(
            dataset=dataset, binary=binary, pca_dims=pca_dims)
    else:
        num_channels = 1
        image_size = 28
    ...
    if pca_dims == 0:
        ...
        train_images = np.reshape(train_images, image_shape)
        test_images = np.reshape(test_images, image_shape)

    init_train_samples=train_labels.shape[0]

    if data_amount > 1.0:
        train_images=train_images[:data_amount]
        train_labels=train_labels[:data_amount]
    else:
        train_images=train_images[:round(init_train_samples*data_amount)]
        train_labels=train_labels[:round(init_train_samples*data_amount)]

    if do_attack:
        if public_data_amount>1.0:
            public_images, public_labels=train_images[:public_data_amount], train_labels[:public_data_amount]
            private_images,private_labels=train_images[public_data_amount:-(attack_train_samples+attack_test_samples)], train_labels[public_data_amount:-(attack_train_samples+attack_test_samples)]
        else:
            public_images, public_labels=train_images[:round(train_images.shape[0]*public_data_amount)], train_labels[:round(train_labels.shape[0]*public_data_amount)]
            private_images,private_labels=train_images[round(train_images.shape[0]*public_data_amount):-(attack_train_samples+attack_test_samples)], train_labels[round(train_labels.shape[0]*public_data_amount):-(attack_train_samples+attack_test_samples)]
        attack_train_images,attack_train_labels=train_images[-(attack_train_samples+attack_test_samples):-attack_test_samples], train_labels[-(attack_train_samples+attack_test_samples):-attack_test_samples]
        attack_test_images,attack_test_labels=train_images[-attack_test_samples:], train_labels[-attack_test_samples:]
    else:
        if public_data_amount > 1.0:
            public_images, public_labels=train_images[:public_data_amount], train_labels[:public_data_amount]
            private_images,private_labels=train_images[public_data_amount:], train_labels[public_data_amount:]
        else:
            public_images, public_labels=train_images[:round(train_images.shape[0]*public_data_amount)], train_labels[:round(train_labels.shape[0]*public_data_amount)]
            private_images,private_labels=train_images[round(train_images.shape[0]*public_data_amount):], train_labels[round(train_labels.shape[0]*public_data_amount):]

    if do_attack:
        return (test_images,test_labels,private_images,private_labels,public_images,public_labels,attack_train_images,attack_train_labels,attack_test_images,attack_test_labels)
    else:
        return (test_images,test_labels,private_images,private_labels,public_images,public_labels)